...
class Subspace(nn.Module):
    ...
    def __init__(self, in_size, out_size):
        ...
        self.shortcut = nn.Conv2d(in_size, out_size, kernel_size=1, bias=True)
        # print('Subspace in_size', in_size, out_size)
...
class UNetConvBlock(nn.Module):
    ...
    def __init__(self, in_size, out_size, downsample, relu_slope):
        ...
        # print(in_size, out_size, downsample, relu_slope)
        self.block = nn.Sequential(
...
class Projection(nn.Module):
    def __init__(self,in_size, subspace_dim=16):
        super(Projection, self).__init__()
        self.num_subspace = subspace_dim
        self.subnet = Subspace(in_size, self.num_subspace)
        # print('Projection in_size', in_size)
    ...
    def forward(self, up, bridge):
        ...
        # print(out.shape)
        # b_, c_, h_, w_ = bridge.shape
        b_, n_, c_ = bridge.shape
        h_ = int(math.sqrt(n_))
        w_ = int(math.sqrt(n_))
        
        out = out.reshape(b_, -1, h_, w_)
        ...
        bridge_ = bridge.reshape(b_, c_, h_*w_)
        project_feature = torch.matmul(project_mat, bridge_.transpose(1, 2))
        bridge = torch.matmul(V, project_feature).transpose(1, 2).reshape(b_, n_, c_)
        # print(bridge.shape, up.shape)
        out = torch.cat([up, bridge], 2)
        
        return out
@ARCH_REGISTRY.register()
class uformerPro(nn.Module):
    def __init__(self, img_size=128, img_ch=3,output_ch=3,
        ...
        self.num_subspace = int(embed_dim / 2)
        ...
        # print('projection_0 : ',embed_dim*16)
        self.projection_0 = Projection(embed_dim*16, self.num_subspace)
        ...
        # print('projection_1 : ',embed_dim*8)
        self.projection_1 = Projection(embed_dim*8, self.num_subspace)
        ...
        self.projection_2 = Projection(embed_dim*4, self.num_subspace)
        ...
        self.projection_3 = Projection(embed_dim*2, self.num_subspace)
    ...
    def forward(self, x, mask=None):
        ...
        deconv0 = self.projection_0(up0,conv3)
    
        deconv0 = self.decoderlayer_0(deconv0,mask=mask)
        
        up1 = self.upsample_1(deconv0)
        # deconv1 = torch.cat([up1,conv2],-1)
        deconv1 = self.projection_1(up1,conv2)
        
        deconv1 = self.decoderlayer_1(deconv1,mask=mask)

        up2 = self.upsample_2(deconv1)
        # deconv2 = torch.cat([up2,conv1],-1)
        deconv2 = self.projection_2(up2,conv1)
        
        deconv2 = self.decoderlayer_2(deconv2,mask=mask)

        up3 = self.upsample_3(deconv2)
        # deconv3 = torch.cat([up3,conv0],-1)
        deconv3 = self.projection_3(up3,conv0)
        
        deconv3 = self.decoderlayer_3(deconv3,mask=mask)
        # print(up0.shape, up1.shape, up2.shape,  up3.shape)
        # print(deconv0.shape, deconv1.shape, deconv2.shape,  deconv3.shape)
        # print(conv0.shape, conv1.shape, conv2.shape,  conv3.shape)
